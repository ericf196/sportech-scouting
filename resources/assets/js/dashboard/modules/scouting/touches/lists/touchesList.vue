<template>
    <div class="box box-solid ">
        ...
        <div class="box-body">
            <ul class="todo-list ui-sortable" v-if="touches.length">
                <li v-for="touch in touches">
                    ...
                    <small class="label label-success">
                        ...
                        {{$t('scoutings.start_at',{start:toHHMMSS(touch.start)})}}
                    </small>
                    <small class="label label-danger">
                        <i class="fa fa-clock-o"></i>
                        {{$t('scoutings.end_at',{end:toHHMMSS(touch.end)})}}
                    </small>
            ...
            <h3 v-if="!touches.length" class="text-center">{{$t('scoutings.no_touches')}}</h3>
        </div>
    </div>
</template>
<style>
