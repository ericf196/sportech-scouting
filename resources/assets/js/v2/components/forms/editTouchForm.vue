                    ...
                    <div class="alert alert-danger">
                        ...
                        <ul>
                            <li v-for="collideTouch in collideTouches">
                                <strong v-if="collideTouch.previous.id != touch.id">{{collideTouch.previous.text}}: that starts
                                    on {{toHHMMSS(collideTouch.previous.start)}} an
                                    ends on
                                    {{toHHMMSS(collideTouch.previous.end)}}</strong>
                                <strong v-if="collideTouch.current.id != touch.id">{{collideTouch.current.text}}: that starts on
...
<script>
    ...
    export default{
        ...
        methods: {
            ...
            checkCollide(){
                var self = this;
                var actions = _.reject(getState('actionManager').actions, function (o) {
                    return self.touch.id == o.id;
                });
